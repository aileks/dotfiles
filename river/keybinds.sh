...
riverctl map normal Super+Shift R spawn "wf-recorder -g '$(slurp)' -f ~/recording_$(date +%Y-%m-%d_%H-%M-%S).mp4 &> /dev/null & echo $! > /tmp/wf-recorder.pid"
riverctl map normal Super+Shift Escape spawn "kill -SIGINT $(cat /tmp/wf-recorder.pid 2>/dev/null) || pkill wf-recorder; rm -f /tmp/wf-recorder.pid"
