...
(defun aileks/org-babel-tangle-config ()
...
(defun aileks/config-window-by-class ()
  (pcase exwm-class-name
  ("Thunderbird" (exwm-workspace-move-window 4))))

(defun aileks/exwm-update-class ()
...
(use-package exwm
  ...
  (add-hook 'exwm-update-class-hook #'aileks/exwm-update-class)

  ;; Call function to set specific workspaces for specific applications
  (add-hook 'exwm-update-class-hook #'aileks/config-window-by-class)
  ...
  ;; Load after-init settings
  (add-hook 'exwm-init-hook #'aileks/after-exwm-init)
...
(defun aileks/run-in-background (command)
  (let ((command-parts (split-string command "[ ]+")))
    (apply #'call-process `(,(car command-parts) nil 0 nil ,@(cdr command-parts)))))

(defun aileks/set-wallpaper ()
  (interactive)
  (start-process-shell-command
      "feh" nil  "feh --bg-scale ~/.bg.png"))

(defun aileks/lockscreen ()
  (interactive)
  (start-process-shell-command
      "xss-lock" nil "xss-lock -- ~/.config/scripts/lock.sh --nofork &"))

(defun aileks/after-exwm-init ()
  ...
  (aileks/run-in-background "nm-applet")
  (aileks/run-in-background "pasystray"))
